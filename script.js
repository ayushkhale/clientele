...
function startCarousel() {
  scrollInterval = setInterval(() => {
    // Animate the first review box out
    const firstBox = carousel.children[0];
    const boxWidth = firstBox.offsetWidth + 30; // Box width + margin
    carousel.style.transition = 'transform 0.5s ease-in-out';
    carousel.style.transform = `translateX(-${boxWidth}px)`;

    // Once the animation ends, reset the position and move the first box to the end
    setTimeout(() => {
...
// Start the carousel when the content is loaded
